...
void LoadHDF5FileToBlob(const char* filename,const string&  names,
  ...
  hdf5_load_nd_dataset(file_id, names.c_str(),
      MIN_DATA_DIM, MAX_DATA_DIM, &hdf_blobs, true);
  herr_t status = H5Fclose(file_id);
  CHECK_GE(status, 0) << "Failed to close HDF5 file: " << filename;
}

template  void LoadHDF5FileToBlob(const char* filename,const string&  names, 
  Blob<float> &hdf_blobs);
template  void LoadHDF5FileToBlob(const char* filename,const string&  names, 
  Blob<double> &hdf_blobs);
